...
__global__ void segmented_spmv_opt(const int nnz, const int * rows, const int * cols, const float * A, const float * x, float * y, int * row_buf0, float * val_buf0, int * row_buf1, float * val_buf1){
 ...
 * same kernel name */
    ...
    while (num_blocks > 1) {
        for (int j = 0; j < red_iter; j++) 
        {
            ...
            int red_blockid = blockIdx.x + j * gridDim.x;
            ...
            if (buf_to_use == 0) {
                ...
                if (red_prodid < red_nnz) {
                    ...
                    filter_sums(red_nnz, red_prodid, red_blockid, r, v, y, row_buf1, val_buf1);
                    __syncthreads();
                }
                ...
                if (red_prodid < red_nnz) {
                    ...
                    // Writing partial sums whose partition is entirely in the block to output
                    // and storing sums who need to be reduced
                    filter_sums(red_nnz, red_prodid, red_blockid, r, v, y, row_buf0, val_buf0);
                    __syncthreads();
                }
            }
        }
    ...
    if (num_blocks == 1) {
        ...
        int last_prodid = thread_id;

        if (buf_to_use == 0) {
            if (last_prodid < red_nnz) {
                v[threadIdx.x] = val_buf0[last_prodid];
                r[threadIdx.x] = row_buf0[last_prodid];

                __syncthreads();
                
                segmented_scan(last_prodid, red_nnz, last_prodid, r, v);
                __syncthreads();

                filter_sums(red_nnz, last_prodid, 0, r, v, y, row_buf1, val_buf1);
                __syncthreads();

                y[row_buf1[1]] = val_buf1[1];
                y[row_buf1[0]] = val_buf1[0];
            ...
            if (last_prodid < red_nnz) {
                ...
                y[row_buf0[1]] = val_buf0[1];
                y[row_buf0[0]] = val_buf0[0];
...
*/
    ...
    segmented_spmv_opt<<<blockNum, blockSize>>>(mat->nz, d_rIndex, d_cIndex, d_val, d_vec, d_out_vec, d_row_buf0, d_val_buf0, d_row_buf1, d_val_buf1);
    ...
    //cudaMemcpy(mat->rIndex, d_rIndex, mat->nz * sizeof(int), cudaMemcpyDeviceToHost);
    //for (int i = 0; i < mat->nz; i++) {
    //    printf("%d\t%d\n", i, mat->rIndex[i]);
    //}
